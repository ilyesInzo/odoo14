...
class AnalyticLine(models.Model):
    ...
    @api.model
    def read_grid(self, row_fields, col_field, cell_field, domain=None, range=None, readonly_field=None, section_field=None, extra_data=None, orderby=None):
        result = super(AnalyticLine, self).read_grid(row_fields, col_field, cell_field, domain, range, readonly_field, section_field, extra_data, orderby)
        # Add call-on duty informations
        start_date = result['cols'][0].get('date', False)
        end_date = result['cols'][-1].get('date', False)
        if not section_field:

            callOnDuties = self.env['call.duty'].search(['&','&',
                                   ('date', '>=', start_date),
                                   ('date', '<=', end_date),
                                   ('user_id', '=', self.env.uid)])

            for callDuty in callOnDuties:
                for col in result['cols']:
                    if str(col.get('date', False)) == str(callDuty.date) and callDuty.isOnCallDuty:
                        col['isOnCallDuty'] = True
                        #print(callDuty.date)
        elif section_field == "employee_id":
            #print(result)
            employeeId = None
            try:
                employeeId = extra_data['sectionGroup']['employee_id'] and extra_data['sectionGroup']['employee_id'][0]
            except:
                pass
    ...
    def action_submit(self):
        ...
        if analytic_lines:
            ...
            notification['params'].update({
                'title': _("There are no timesheets to submit"),
                'type':'warning' 
            })
            return notification
...
class callDutyDay(models.Model):

    _name = "call.duty"
    date = fields.Date('Date', index=True)
    isOnCallDuty = fields.Boolean('Is on-Call duty')
    ...
    @api.model
    def action_on_call_duty(self, arg):
        date = arg['date']
        callDuty = self.search([('date','=',date),('user_id', '=', self.env.uid)], limit=1)
        # create if not exist
        if not callDuty:
            employee = self.env['hr.employee'].search([('user_id', '=', self.env.uid)], limit=1)
            self.sudo().create({'date': date,
        'isOnCallDuty': arg['isOnCallDuty'],
        'user_id':self.env.uid,
        'employee_id': employee.id if employee else None})
        else:
            callDuty.write({'date': date,
        'isOnCallDuty': arg['isOnCallDuty'],
        'user_id':self.env.uid})
